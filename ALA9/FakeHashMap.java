...
//         this(100, 0.9);
//     }
//     public FakeHashMap(int c) {
//         this(c, 0.9);
//     }
//     public FakeHashMap(int c, double lf) {
...
public class FakeHashMap <K, V>{
    ...
    public FakeHashMap() {  // O(1)
        this(100, 0.9);
    }

    public FakeHashMap(int c) { // O(log(n))
        this(c, 0.9);
    }

    public FakeHashMap(int c, double lf) {  // O(log(n))
        hashTable = new LinkedList[trimToPowerOf2(c)];
        loadFactor = lf;
        size = 0;
        collisions = 0;
    ...
    public V put(K key, V value) {  //O(1) if not rehashing, O(n) if rehashing
        if(get(key) != null) { // The key is in the hash map
            int HTIndex = hash(key.hashCode());
            LinkedList<HashMapEntry<K,V>> ll;
            ll = hashTable[HTIndex];
